...
struct GameState {
    ...
    bool point_segment(pair<int, int> a, pair<int, int> b, pair<int, int> c) {
        double sum = dist(b, a) + dist(a, c) - dist(b, c);
                ...
                for (int j = 0; j < modules_count; ++j) {
                    ...
                    if (buildings[dst].tubes_ids.size() != MAX_TUBES) {
                        ...
                        if (!tube_exists(src, {src, dst}) && !collides({src, dst})) {
                            type = modules[j].type;
                            curr = resources > 20000 ? pad_heur(src, type) : astronauts_heur(src, type);
    ...
    Line connecting_tube() {
        ...
        int pads_count = pads.size();
        int max = 0, src, dst, curr, type, cost, new_dst, id;

        for (int i = 0; i < pads_count - 1; ++i) {
            src = pads[i].id;

            if (buildings[src].tubes_ids.size() != MAX_TUBES) {
                for (int j = i + 1; j < pads_count; ++j) {
                    dst = pads[j].id;
                    ...
                    if (buildings[dst].tubes_ids.size() != MAX_TUBES) {
                        ...
                        if (!tube_exists(src, {src, dst}) && !collides({src, dst})) {
                            ...
                            for (const int id : buildings[dst].tubes_ids) {
                                new_dst = tubes[id].src == dst ? tubes[id].dst : tubes[id].src;
                                type = buildings[new_dst].type;

                                if (type != -1) {
                                    curr += pads[i].astronauts[type];
                                }
                            }
                            id = buildings[dst].portal_id;
                            if (id != -1) {
                                new_dst = portals[id].src == dst ? portals[id].dst : portals[id].src;
                                type = buildings[new_dst].type;
    ...
    void solve() {
        ...
        while (place_tubes || place_portals) {
            if (place_tubes) {
                Line new_tube = optimal_tube();

                if (new_tube.src == -1) {
                    new_tube = connecting_tube(); 
                }
                ...
                Line new_portal = optimal_portal();
